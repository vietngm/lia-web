...
<main>
		<div  class="section-advise py-4" style="padding-top:5rem">
			<div class="container">
				...
				<div class="highlight-filter pt-2 pb-3 bg-white" data-required="true">
					...
					<div class="item <?= !$index ? "item-active" : "" ?>" data-id="<?= $data["service-category"]->term_id ?>">
						<img style="width: 1.875rem;" src="<?= get_field("featured_image", "service-category_{$data["service-category"]->term_id}") ?>" />
				...
				<div class="content" data-id="1">
					...
					<div class="advise-age">
						...
						<div class="flex flex-wrap gap-2 mb-4 input-advise-age">
							...
							<label class="check-button">
								<input type="radio" name="age" <?= !$data_age_index && !$age_index ? "checked" : "" ?> value="<?= str_replace(" ", "", $age) ?>" />
								<span class="label">
					...
					<div class="advise-status">
						...
						<div class="flex gap-4 overflow-x-auto no-scrollbar mb-4 input-advise-status">
							...
							<?php foreach($data_statuses["data"] as $data_status): foreach ($data_status["status"] as $status) : ?>
								...
								<label class="flex-shrink-0 cursor-pointer border-1  relative" style="box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;margin-bottom: 24px;margin-left: 10px;border-radius: 10px;">
					...
					<div class="advise-desire hidden">
						...
						<p class="mb-4">Vui lòng chọn tình trạng hiện tại của bạn , để biết thêm chi tiết từng mong muốn đề xuất phù hợp.</p>
			...
			<div class="advise-footer hidden">
				...
				<div class="h-[80px] flex items-center border-t-1 border-[#ccc] fixed bottom-0 left-0 right-0 bg-white bottom-action " style="border-top:1px solid #eee; z-index:10">
					<div class="container">
