...
<style>
			.modal-desire,.modal-material,.modal-bh,.modal-topping,.modal-pd {
		...
		.modal-desire.show,.modal-material.show,.modal-bh.show,.modal-topping.show,.modal-pd.show {
		...
		.overlay {
			...
			left: 0;
			width: 100%;
			height: 7rem; /* h-28 trong Tailwind là 7rem */
		...
		body.modal-open {
			...
			width: 100vw; /* Đảm bảo không bị tràn */
			height: 100vh;
			top: 0;
			left: 0;
			right: 0;
        ...
        .option-desire, .option-material, .option-bh {
            display: flex;
            cursor: pointer;
            position: relative;
            width: 100%;
        ...
        .option-desire img, .option-material img, .option-bh img {
            width: 50px;
            height: 50px;
            object-fit: cover;
        }

        .option-desire .text, .option-material .text, .option-bh .text {
            margin-left: 10px;
            flex-grow: 1;
        }

        .option-desire .text p, .option-material .text p, .option-bh .text p {
            margin: 2px 0;
            font-size: 14px;
            color: #333;
        }

        .option-desire .price, .option-material .price, .option-bh .price {
            color: red;
            font-weight: bold;
        }

        /* Khi được chọn */
        .option-desire.selected, .option-material.selected, .option-bh.selected {
            border-color: #8C61A8;  /* Viền tím */
			border-width: 1.5px;
        }
		.option-desire.selected::after, .option-material.selected::after, .option-bh.selected::after {
			content: "✔";
			position: absolute;
			bottom: 2px;
			right: 2px;
		...
		.image_service img , .contentBox img {
			width: 100%;
		}
		.swiper-wrapper{
			height:auto;
...
<main>
			...
			<div class="grid grid-cols-2 gap-4 relative">
				<div class="product-detail-slider mount-slider lg:col-span-1 col-span-2 sm:mt-0 sm:mx-0  -mx-4">
					<?php foreach ($fields["gallery"] as $item) : ?>
						<div>
							<img class="w-full" style="margin-top:0px" src="<?= bfi_thumb($item , array("width"=>800, 'crop'=>false)) ?>" />
						</div>	
					<?php endforeach; ?>
				</div>
				<div class="lg:col-span-1 col-span-2 flex flex-col">
					...
					<div class=" flex items-center gap-1">
						...
						<span class="text-12"><?= $fields["time"] ?> 60 phút</span>
					</div>
					<div class="mt-2">
	...
	<section>
		<div class="max-w-md mx-auto bg-white p-4 rounded-lg shadow-lg " style="padding-top:0px;padding-bottom:0px">
			<h2 class="form-title text-lg font-semibold border-l-4 border-purple-500 pl-2" style="font-size:16px;color:#1A5477">Chọn topping</h2>
			<?php if (!empty($fields['desire'])): ?>
			<div class="mt-2">
				<div id="priceData" data-price="<?= isset($fields['discountPrice']) && $fields['discountPrice'] > 0 && $fields['discountPrice'] < $fields['price'] ? $fields['discountPrice'] : (isset($fields['price']) ? $fields['price'] : 0); ?>"></div>
				<div class="flex justify-between items-center">
				<h3 class="font-medium">
					<?= !empty($fields['name_desire']) ? $fields['name_desire'] : "Hình thức gội đầu" ?> <span class="text-12" style="font-weight:300"> ( Chọn 1 ) </span>
				</h3>
					<button id="desire" class="submit text-sm text-gray-500 cursor-pointer" style="font-size:12px;color:#646464;font-style:italic"><img class="w-3 h-3" src="<?= get_theme_file_uri("assets/images/icons/chamthan.svg") ?>" alt="" /></button>
				</div>
				<div class="grid grid-cols-2 gap-3 mt-2 options-container overflow-x-auto no-scrollbar">
					...
					<?php foreach ($visible_toppings as $desire_topping) : ?>
						...
						<div id="mainPageDesire" data-name="<?= $term->name ?>" data-price="<?= $desire_topping["origin"] ?>" 
							...
							class="option-desire flex items-center relative cursor-pointer gap-2" 
			...
			<div class="mt-2">
				<div class="flex justify-between items-center">
				<h3 class="font-medium">
					<?= !empty($fields['name_material']) ? $fields['name_material'] : "Topping A" ?><span class="text-12" style="font-weight:300"> ( Chọn 1 ) </span>
				</h3>
					<button id="material" class="submit text-sm text-gray-500 cursor-pointer" style="font-size:12px;color:#646464;font-style:italic"><img class="w-3 h-3" src="<?= get_theme_file_uri("assets/images/icons/chamthan.svg") ?>" alt="" /></button>
				</div>
				<div class="gap-2 mt-2 flex flex-col overflow-x-auto no-scrollbar options-container w-full">
					...
					<?php foreach ($visible_toppings_material as $material_topping) : ?>
						...
						?>
						<div  data-name="<?= $term->name ?>" data-price="<?= $material_topping["origin"] ?>"  class="option-material  flex-col flex text-sm cursor-pointer "  onclick="selectOptionMaterial(this)">
							<div style="overflow:hidden">
								<label class="radio">
			...
			<div class="mt-2">
				<div class="flex justify-between items-center">
					...
					<button id="bh" class="submit text-sm text-gray-500 cursor-pointer" style="font-size:12px;color:#646464;font-style:italic"><img class="w-3 h-3" src="<?= get_theme_file_uri("assets/images/icons/chamthan.svg") ?>" alt="" /></button>
				</div>
				<div class="flex flex-col gap-2 mt-2 ">
					...
					<?php foreach ($visible_toppings_bh as $bh_topping) : ?>
						...
						?>
						<div  data-name="<?= $term->name ?>" data-price="<?= $bh_topping["origin"] ?>"  class="option-bh flex-col flex  text-sm cursor-pointer "  onclick="selectOptionBh(this)">
			...
			<div class="section-luuy mt-2">
				<h3 style="font-weight:500">Thêm lưu ý <span class="text-12" style="font-weight:300"> ( Không bắt buộc ) </span></h3>
			...
			<div class="mt-4 text-center">
				...
				<div id="modal-topping" class=" modal-topping fixed hidden top-0 left-0 right-0 bottom-0 z-[120]  modal-popup" style="z-index: 1000">
					<?php 
						set_query_var('field', $fields);
	...
	<section>
		<div class="max-w-2xl mx-auto bg-white p-4 rounded-lg shadow-md" style="padding-top:0px">
			...
			<h2 class="form-title text-lg font-semibold border-l-4 border-purple-500 pl-2" style="font-size:16px;color:#1A5477">Chuyên viên</h2>
			<div class="flex overflow-x-auto no-scrollbar gap-3 mt-2">
				...
				<a href="<?= get_permalink($doctorId) ?>" style="background:#F6F6F6" class="flex flex-shrink-0 gap-3 px-3 py-2 rounded-2">
			...
			<div class="reviews-container">
				...
				<div class="reviews flex overflow-x-auto no-scrollbar gap-3">
					<div class="review-card">
						...
						<p>Vitae Nam tempor viverra quis vel dui malesuada. Cras odio ultrices dignissim, odio viverra luctus vel nisl...</p>
					</div>
	...
	<secrion>
		<div class="container">
			<div>
			<h2 class="form-title text-lg font-semibold border-l-4 border-purple-500 pl-2" style="font-size:16px;color:#1A5477">Cam kết chính sách</h2>
			...
			<div class="mt-4">
			<h2 class="form-title text-lg font-semibold border-l-4 border-purple-500 pl-2" style="font-size:16px;color:#1A5477">Mô tả chi tiết</h2>
	...
	<section>
		<div class="container">
			<?php if ($the_query_related->have_posts()) : ?>
				<h2 class="form-title text-lg font-semibold border-l-4 border-purple-500 pl-2" style="font-size:16px;color:#1A5477">Có thể bạn quan tâm</h2>
...
function addClassToMainPage() {
	...
	<script>
		document.querySelectorAll('.option-desire').forEach(option => {
			...
			localStorage.setItem('selectedDesire', JSON.stringify({ name, price }));
			updateUI();
			updateNoteTopping();
	...
	<script>
		document.querySelectorAll('.option-material').forEach(option => {
			...
			localStorage.setItem('selectedMaterial', JSON.stringify({ name, price }));
			updateUI();
			updateNoteTopping();
	...
	<script>
		document.querySelectorAll('.option-bh').forEach(option => {
			...
			localStorage.setItem('selectedBh', JSON.stringify({ name, price }));
			updateUI();
			updateNoteTopping();
...
function addOptionEvent(selector, storageKey) {
    document.querySelectorAll(selector).forEach(option => {
        option.addEventListener("click", function () {
            ...
            localStorage.setItem(storageKey, JSON.stringify({ name, price }));
...
function updateCheckedState(selector, name) {
    document.querySelectorAll(selector).forEach(input => {
		...
		function updateTotalPrice() {
			...
			const selectedDesire = JSON.parse(localStorage.getItem("selectedDesire")) || { price: 0 };
			const selectedMaterial = JSON.parse(localStorage.getItem("selectedMaterial")) || { price: 0 };
			const selectedBh = JSON.parse(localStorage.getItem("selectedBh")) || { price: 0 };
