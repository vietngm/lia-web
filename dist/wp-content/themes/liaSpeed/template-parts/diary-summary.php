...
<div class="swiper-slide"
  style="margin-right:12px;box-shadow: rgba(0, 0, 0, 0.05) 0px 0px 0px 1px, rgb(209, 213, 219) 0px 0px 0px 1px inset;padding:12px;border-radius:8px;gap:4;width:350px">
  <div style="display: flex; flex-direction: column">
    ...
    <div class="flex gap-2  items-center mt-2">
      ...
      <div style="font-size:12px"><?php if ($services && count($services)) : ?> <?php   $service = $services[0]; ?>
        <a href="<?= get_permalink($service) ?>" class="flex-shrink-0  ">
          <div>Đã chia sẻ nhật ký làm đẹp dịch vụ <b class="text-12 font-bold"><?= get_the_title($service) ?></b> được
            <b><?= get_the_title($fields["doctor"]) ?> </b> phụ trách điều trị tại <b>PK Trang Beauty Center</b> </div>
        </a>
        <?php endif; ?>
      </div>
    </div>
    <div class="mt-2 flex gap-2 mb-4">
      ...
      <div class="no-scrollbar flex overflow-x-auto  ">
        ...
        <a href="<?= get_permalink($service) ?>" class="flex-shrink-0 flex-1 gap-3 flex items-center ">
          <img style="width:40px" class=" rounded-1"
            src="<?= bfi_thumb(get_the_post_thumbnail_url($service) , array("width"=>400, 'crop'=>false)) ?>" alt="">
          <div class="flex-1">
            <h4 class=" text-12 mb-0.5 font-semibold"
              style="  width: 100px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
              <?= get_the_title($service) ?></h4>
      ...
      <div class="flex gap-2 items-center ">
        <img style="width:40px" class=" rounded-1 border-1 border-[#ccc]"
          src="<?= get_the_post_thumbnail_url($fields["doctor"]) ?>" />
        <div class="flex-1">
          <h4 class="text-12 mb-0.5 font-semibold"
            style="  width: 100px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
            <?= get_the_title($fields["doctor"]) ?></h4>
          <div class="flex items-center gap-2 text-10 mb-0.5">
            <img class="w-3.5 h-3.5" src="<?= get_theme_file_uri("assets/images/icons/star-black.svg") ?>" />
    ...
    <div class="mb-2"
      style="font-size:13px; display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp:3; overflow: hidden;">
      <?= $fields["description"] ?>
    </div>
  </div>
  <div>
    <div class="flex gap-2 mb-4">
      <div class="flex-1">
        <a href="<?= $fields["image"]["before"] ?>" data-fancybox>
          <img class="w-[130px h-[130px] rounded-1 object-cover object-center"
            src="<?= bfi_thumb($fields["image"]["before"] , array("width"=>600, 'crop'=>false)) ?>" />
        </a>
      ...
      <div class="flex-1">
        <a href="<?= $fields["image"]["after"] ?>" data-fancybox>
          <img class="w-[130px h-[130px]  rounded-1 object-cover object-center"
            src="<?= bfi_thumb($fields["image"]["after"] , array("width"=>600, 'crop'=>false)) ?>" />
        </a>
    ...
    <div class="processing-group">
      ...
      <div
        ...
        <div
          style="font-size:13px; display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp:2; overflow: hidden;">
          <?= $processing["description"] ?></div>
        <div class="mt-2 flex flex-wrap gap-2">
          <?php foreach ( $processing["images"] ?? [] as $image ) : ?>
          <a class="w-[80px] h-[80px] block" href="<?= $image ?>">
            <img class="w-full h-full rounded-1" src="<?= bfi_thumb($image, array("width" => 200, 'crop' => false)) ?>"
              alt="" />
          </a>
          <?php endforeach; ?>
        </div>
      ...
      <div style="display: flex; justify-content: flex-end;">
        <button class="show-more-btn mt-3 px-4 py-2 bg-blue-500 rounded"
          style="padding: 4px 14px; background: #7c2c8d; color: #FFF;">Xem chi tiết</button>
      </div>
      <?php endif; ?>
    </div>
